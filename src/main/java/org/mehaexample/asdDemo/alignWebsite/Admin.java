...
public class Admin{
	...
	ElectivesAdminDao electivesAdminDao = new ElectivesAdminDao(true);
	...
	@Path("/{nuid}/gpa")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getStudentGPA(@PathParam("nuid") String nuid) {
		List<ElectivesAdmin> electivesAdmins;
		ArrayList<String> gpaList = new ArrayList<>();
		if (!studentDao.ifNuidExists(nuid)) {
			return Response.status(Response.Status.NOT_FOUND).entity("No Student record exists with given ID").build();
		} else {

			electivesAdmins = electiveDao.getElectivesByNeuId(nuid);

			for(ElectivesAdmin electivesAdmin : electivesAdmins){
				gpaList.add(electivesAdmin.getCourseName()+":"+ electivesAdmin.getGpa());
			}
		}
		JSONObject jsonObj = new JSONObject(gpaList);
		jsonObj.put("elective", gpaList);
		return Response.status(Response.Status.OK).entity(jsonObj.toString()).build();
	}
